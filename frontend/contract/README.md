...
| `layers.contract.ts`   | #117 S2 | event **channel layers**: `GET/POST/PUT/DELETE /events/{id}/layers` serve exactly `bindings/ChannelLayers.ts` (the expectation **generated** from the binding, never hand-written); the global→event **seed** (a create with no `nodes` comes back tuned from the timer's allowed set, and the timer record is byte-identical afterwards — layers are event state, not a timer edit); the one **error** (two nodes on one channel → 400 naming both nodes and the channel by friendly name, plus off-pool channel / phantom node / incomplete tuning) against the one **warning** (channel reuse _between_ layers → a **200** carrying `overlaps`); the RD gate on writes with an open read |
