...
describe('#418 — a round with only unstarted heats deletes, heats included', () => {
  it('deletes the round and stops serving its heats', async () => {
    ...
    const filled = await listHeats(director.baseUrl, PRACTICE_EVENT_ID);
    ...
    const meta = await deleteRound(director.baseUrl, PRACTICE_EVENT_ID, round.id, TOKEN);
  ...
  it('still refuses a round with a heat IN PROGRESS, names it, and advises nothing impossible', async () => {
    ...
    const filled = await rdControl(director.baseUrl, TOKEN, {
      FillRound: { round: round.id, mode: 'Next' }
    });
    ...
    const selected = await rdControl(director.baseUrl, TOKEN, {
      SetCurrentHeat: { heat: heat.heat }
    });
    expect(selected.ok).toBe(true);
    const stage = await rdControl(director.baseUrl, TOKEN, { Stage: { heat: heat.heat } });
    expect(stage.ok).toBe(true);

    await expect(
      deleteRound(director.baseUrl, PRACTICE_EVENT_ID, round.id, TOKEN)
    ).rejects.toThrow();

    // The refusal itself: names the blocking heat by its friendly name, and recommends no route.
    const resp = await fetch(`${director.baseUrl}/events/${PRACTICE_EVENT_ID}/rounds/${round.id}`, {
      method: 'DELETE',
    ...
    await rdControl(director.baseUrl, TOKEN, { Abort: { heat: heat.heat } });
  });
});
...
describe('#416 — a stored round seating a node the timer does not have is reported on READ', () => {
  it('names the round, the timer and the 1-based node — never a raw ref', async () => {
    ...
    await setEventTimers(director.baseUrl, PRACTICE_EVENT_ID, [timer!.id], TOKEN);
    ...
    await expect(
      createRound(director.baseUrl, PRACTICE_EVENT_ID, practiceRound('Impossible', [6]), TOKEN)
    ).rejects.toThrow();

    // A round authored while the event had no timer is the stored shape that slipped through.
    await setEventTimers(director.baseUrl, PRACTICE_EVENT_ID, [], TOKEN);
    const round = await createRound(
      director.baseUrl,
      PRACTICE_EVENT_ID,
    ...
    expect(await listRoundIssues(director.baseUrl, PRACTICE_EVENT_ID)).toEqual([]);

    // Now the timer is back in the picture, and the seat becomes checkable — and impossible.
    await setEventTimers(director.baseUrl, PRACTICE_EVENT_ID, [timer!.id], TOKEN);
    const issues = await listRoundIssues(director.baseUrl, PRACTICE_EVENT_ID);
    ...
    await deleteRound(director.baseUrl, PRACTICE_EVENT_ID, round.id, TOKEN);
  });
});
