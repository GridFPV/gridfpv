...
describe('roundRobinStandings — points aggregation + best lap, ordered by ranking', () => {
  ...
  it('sums the points table per finish across heats and renders in ranking order', () => {
    ...
    const rows = roundRobinStandings(ranking, heats, POINTS, label, (id) => byHeat[id]);
    ...
    // Best lap is the MIN across the pilot's heats.
  ...
  it('falls back to lineup order with zero points when the round has no ranking yet', () => {
    ...
    const rows = roundRobinStandings([], heats, POINTS, label, () => undefined);
    expect(rows.map((r) => [r.label, r.position, r.points])).toEqual([
      ['Bolt', 1, 0],
      ['AceOne', 2, 0]
    ]);
