...
describe('marshaling command builders', () => {
  it('voidDetectionCommand targets a log offset', () => {
    expect(voidDetectionCommand(7n)).toEqual({ VoidDetection: { target: 7n } });
  });

  it('insertLapCommand carries adapter, competitor, and source time', () => {
    expect(insertLapCommand('rh-1', 'ALICE', 1_000_000n)).toEqual({
      InsertLap: { adapter: 'rh-1', competitor: 'ALICE', at: 1_000_000n }
    });
  });

  it('adjustLapCommand re-times a logged pass', () => {
    expect(adjustLapCommand(3n, 2_000_000n)).toEqual({ AdjustLap: { target: 3n, at: 2_000_000n } });
  });
  ...
  it('converts whole seconds to microsecond SourceTime', () => {
    expect(secondsToSourceTime(1.5)).toBe(1_500_000n);
  });
});
